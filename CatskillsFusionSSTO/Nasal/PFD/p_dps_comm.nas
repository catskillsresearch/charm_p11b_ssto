...
var PFD_addpage_p_dps_comm = func(device)
{
	...
	p_dps_comm.pwr_out_arrow = device.svg.getElementById("p_dps_comm_pwr_out_arrow");
	p_dps_comm.pwr_out_arrow.enableUpdate();

	p_dps_comm.pwr_out1_arrow = device.svg.getElementById("p_dps_comm_pwr_out1_arrow");
    p_dps_comm.pwr_out2_arrow = device.svg.getElementById("p_dps_comm_pwr_out2_arrow");
	p_dps_comm.pwr_out1_arrow.enableUpdate();
	p_dps_comm.pwr_out2_arrow.enableUpdate();

	p_dps_comm.temp1_arrow = device.svg.getElementById("p_dps_comm_temp1_arrow");
    p_dps_comm.temp2_arrow = device.svg.getElementById("p_dps_comm_temp2_arrow");
	p_dps_comm.temp1_arrow.enableUpdate();
	p_dps_comm.temp2_arrow.enableUpdate();

	p_dps_comm.pwr_level_ssor_arrow = device.svg.getElementById("p_dps_comm_pwr_level_ssor_arrow");
    p_dps_comm.pwr_status_ssor_arrow = device.svg.getElementById("p_dps_comm_pwr_status_ssor_arrow");
	p_dps_comm.pwr_level_ssor_arrow.enableUpdate();
	p_dps_comm.pwr_status_ssor_arrow.enableUpdate();


	#All arrows in a group for Yellow color

    p_dps_comm.arrow_group = device.svg.getElementById("p_dps_comm_arrow_group");
    p_dps_comm.arrow_group.setColor(1, 1, 0);
	...
	if (power_amp_operational * power_amp_switch == 1)
		...
		p_dps_comm.pwr_out1.setText(sprintf("%3d", int(power_amp_watts) ));
		p_dps_comm.pwr_out1_arrow.updateText("");
		p_dps_comm.pwr_out2.setText("  0");
		p_dps_comm.pwr_out2_arrow.updateText("L");
		p_dps_comm.temp1.setText(sprintf("%3d", int(nose_temp + power_amp_watts + 2)));
		p_dps_comm.temp2.setText(sprintf("%3d", nose_temp));

		# If temp below 32°F, temp alert for S band antenna not working

		if (nose_temp < 32){p_dps_comm.temp2_arrow.enableUpdate("L");}
		else {p_dps_comm.temp2_arrow.enableUpdate("");}
		}
		...
		p_dps_comm.pwr_out1_arrow.updateText("L");
		p_dps_comm.pwr_out2.setText(sprintf("%3d", int(power_amp_watts) ));
		p_dps_comm.pwr_out2_arrow.updateText("");
		p_dps_comm.temp1.setText(sprintf("%3d", nose_temp));
		p_dps_comm.temp2.setText(sprintf("%3d", int(nose_temp + power_amp_watts -4)));

		if (nose_temp < 32){p_dps_comm.temp1_arrow.enableUpdate("L");}
		else {p_dps_comm.temp1_arrow.enableUpdate("");}
		}
	else 
		{
		p_dps_comm.oper1.updateText("");
		...
		if (nose_temp < 32){p_dps_comm.temp1_arrow.enableUpdate("L"); p_dps_comm.temp2_arrow.enableUpdate("L");}
		else {p_dps_comm.temp1_arrow.enableUpdate(""); p_dps_comm.temp2_arrow.enableUpdate("");}
	...
	p_dps_comm.pwr_out.setText(sprintf("%2d", int(ku_power) ));
	...
	if (ku_jettisoned == 0)
		...
		var ku_temp = K_to_F(pb_temp + 0.5 * ku_power);
	    	...
	    	p_dps_comm.pa_temp.setText(sprintf("%+4.0f", ku_temp));
	    	p_dps_comm.gimbal_temp_A.setText(sprintf("%+4.0f", ku_temp-1));
	    	p_dps_comm.gimbal_temp_B.setText(sprintf("%+4.0f", ku_temp-3));
	    	p_dps_comm.gryo_temp.setText(sprintf("%+4.0f", ku_temp+4));
		}
	else
		{
			#No antenna //Just M in Yellow

	    	p_dps_comm.pa_temp.setText("    M");
			p_dps_comm.pa_temp.setColor(1, 1, 0);
	    	p_dps_comm.gimbal_temp_A.setText("    M");
			p_dps_comm.gimbal_temp_A.setColor(1, 1, 0);
	    	p_dps_comm.gimbal_temp_B.setText("    M");
			p_dps_comm.gimbal_temp_B.setColor(1, 1, 0);
	    	p_dps_comm.gryo_temp.setText("    M");
			p_dps_comm.gryo_temp.setColor(1, 1, 0);
		}
	...
	if (uhf_operational == 0)
		{
    		p_dps_comm.pwr_level_ssor.setText("+15.0");
			p_dps_comm.pwr_level_ssor_arrow.updateText("L");
    		p_dps_comm.pwr_status_ssor.setText("0.0");
			p_dps_comm.pwr_status_ssor_arrow.updateText("L");
	...
	else
		{
			...
			p_dps_comm.pwr_level_ssor_arrow.updateText("");
    		p_dps_comm.pwr_status_ssor.setText("+8.0");
			p_dps_comm.pwr_status_ssor_arrow.updateText("");
	...
	if (SpaceShuttle.sm_simulation_detail_level == 1)
		...
		p_dps_comm.arrow_group.setVisible(1);

		#Ku Antenna (L for 0 power)

		if (int(ku_power) == 0){p_dps_comm.pwr_out_arrow.updateText("L");}
		else {p_dps_comm.pwr_out_arrow.updateText("");}


		#S-Band Antenna (L for 0 Power // L for Temp below 32°F) // done directly in the conditionnal line 246 to avoid another complex one here

		#SSOR UHF Power level /Status ( Level L below 15 // Status L below 0) //done directly line 372 to avoid another complex cond here
		}
	else {p_dps_comm.arrow_group.setVisible(0);} 
