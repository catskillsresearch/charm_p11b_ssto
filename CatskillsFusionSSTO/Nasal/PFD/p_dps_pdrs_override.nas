...
var PFD_addpage_p_dps_pdrs_override = func(device)
{
	...
	p_dps_pdrs_override.inputs_inh = device.svg.getElementById("p_dps_pdrs_override_inputs_inh");
	p_dps_pdrs_override.inputs_inh.enableUpdate();
    ...
    p_dps_pdrs_override.safing_can = device.svg.getElementById("p_dps_pdrs_override_saving_can");
	...
	# default options not implemented yet
    	...
    	p_dps_pdrs_override.safing_can.setText("");
	...
	#If RMS inactive, "Inputs Inhibited top message" (Normally when RMS soft not executing)

	if (getprop("/fdm/jsbsim/systems/rms/rms-active") == 0) {p_dps_pdrs_override.inputs_inh.updateText("INPUTS INHIBITED");}
	else {p_dps_pdrs_override.inputs_inh.updateText("");}
