...
io.include("cdlg_eileen.nas");
io.include("cdlg_cuecards.nas");
...
var cdlg_mdu_clone = {
	...
	init: func (index) {
		...
		var bg_canvas = window.createCanvas()
                   .setColorBackground(0,0,0,1);
		
		
		var canvas_clone = SpaceShuttle.MDU_array[index].PFD._canvas;
		#window.setCanvas(canvas_clone);
		var root = bg_canvas.createGroup();
		root.createChild("image")
                                    .setFile( canvas_clone.getPath() )
                                    .setSize(512,512)
                                    .setTranslation(0,0);
