...
var cdlg_traj_map = {
	...
	init: func {
		...
		if (SpaceShuttle.n_orbital_targets > 0) 
			{
			me.sym_otgt_label.setText(SpaceShuttle.oTgt.label);
		...
		me.num_points_horizon = 0;
		me.history_draw_flag = 0;
		me.num_points_prediction = 0;
	...
	task_landsites: func {
		...
		for (var i=0; i< size(SpaceShuttle.landing_site_data.array); i=i+1)
			{

			var draw_flag = 0;
			var function = SpaceShuttle.landing_site_data.array[i].function;
			...
			if (draw_flag == 1)		
				...
				var x = me.lon_to_x(SpaceShuttle.landing_site_data.array[i].coord.lon());
				var y = me.lat_to_y(SpaceShuttle.landing_site_data.array[i].coord.lat());

				var text = SpaceShuttle.landing_site_data.array[i].shortname;
				var offset = SpaceShuttle.landing_site_data.array[i].text_vertical_offset;
