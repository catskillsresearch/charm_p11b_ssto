...
geo.put_model("Aircraft/CatskillsFusionSSTO/Models/Launchpad/launchpad.xml", lat + delta_lat, lon + delta_lon, nil, heading);

if (getprop("/sim/config/shuttle/place-gantry") == 1)
	{
	settimer( func {geo.put_model("Aircraft/CatskillsFusionSSTO/Models/Launchpad/gantry.xml", lat + delta_lat, lon + delta_lon, nil, heading);}, 8.0);
	}
