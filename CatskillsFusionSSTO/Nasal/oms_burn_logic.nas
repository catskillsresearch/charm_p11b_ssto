...
#Initial Accel of 0.51m.s²/1.5 ft.s² (2OMS burn and 233klbs weight) / accel burn updated to the correct weight/burn selection in create_oms_burn_vector
SpaceShuttle.targeting_manager.set_acceleration(oms_burn_target.accel_burn);

SpaceShuttle.targeting_manager.set_peg4(oms_burn_target.ht * 1852.0, oms_burn_target.thetaT, oms_burn_target.c1 * 0.3048, oms_burn_target.c2, tig - MET);
...
if (SpaceShuttle.targeting_manager.evolution_finished == 0) 
	...
	if (SpaceShuttle.targeting_manager.peg4_fit_converged == 1)
		...
		#Set hash values for peg4 dvx/y/z plan for mnvr.nas display of dvx/y/z if peg 4 converged

		oms_burn_target.dvx_plan = SpaceShuttle.targeting_manager.dvx/0.3048;
		oms_burn_target.dvy_plan = SpaceShuttle.targeting_manager.dvy/0.3048;
		oms_burn_target.dvz_plan = SpaceShuttle.targeting_manager.dvz/0.3048;
...
#Avoid some floating point errors in various places and DPS crash ( Coordinate.nas / Orbital_targeting.nas ) if item 22 depressed with zero as a value in dvx/y/z
if (dvtot == 0) {return;}
...
var acceleration_si = acceleration * 9.806650;

#Accurate accel taking into account Current weight and burn mode (m.s²)

SpaceShuttle.targeting_manager.set_acceleration(acceleration_si);
oms_burn_target.accel_burn = acceleration_si;
...
# At end of burn, no reset of PEG 4 value,ATT burn, neither tgo and dv tot until mm change or Reload
#It allows to have Dv residuals for fine tunning
...
oms_burn_target.dvx_plan = 0;
oms_burn_target.dvy_plan = 0;
oms_burn_target.dvz_plan = 0;
...
#Reset of accel to a mean value for 2 OMS burn
oms_burn_target.accel_burn = 0.51;
...
#Blank functions from command parser // blank hash variables if item 22 canceled /ops recall/ ops changes

SpaceShuttle.blank_peg7();
