...
var imu_unit = {
	...
	update_status : func {
		...
		if ((me.condition == 0) or (me.switch_status == 0))
			{	
			me.operational = 0;
	...
	thermal_evolution: func {
		...
		if (me.temperature > 156.5)
			...
			setprop("/fdm/jsbsim/systems/cws/imu-temp", 1);    # Trigger for Overheat failure message
			}
		else if (me.temperature == 141.0)
			{
			me.temp_string = "LO";
			me.thermal_drift = 0.0;
			setprop("/fdm/jsbsim/systems/cws/imu-temp", 0);
			}
		else
			{
			me.temp_string = "OK";
...
var imu_system = {
	...
	candidate_selection: func {
		...
		if (me.num_cand == 1)
			...
			setprop("/fdm/jsbsim/systems/cws/imu-dlma", 0);   #Trigger for  IMU F7 light dilemma
			}
	...
	redundancy_management_lowlevel: func (data) {
		...
		if (me.num_cand == 3) #mid-value selection
			...
			if (diff > me.deselect_threshold_2)
				...
				setprop("/fdm/jsbsim/systems/cws/imu-dlma", 1);   #Trigger for  IMU F7 light C/W dilemma
