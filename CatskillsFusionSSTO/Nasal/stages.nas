...
#Ascent abort boundary hash values
var ascent_abort_boundary = {

	#Boundary velocites
	two_engines_tal: 5800,
	rtls: 8200,
	ato: 10800,
	two_engines_ptm: 14900,
	hdot_2eo: 1850,
	single_ops3: 12300,
	single_engine_tal: 14300,
	single_engine_ptm: 17600,
	no_engines_tal: 18400,
	no_engines_aoa: 25100,

	#Abort boundary flag
	abort_one_eo_boundary_flag: 0,
	abort_two_eo_boundary_flag: 0,
	tal_redesignation_flag: 0,
	};

#Boundary velocities computation functions

var abort_boundary_update = func{

if (launch_loop_flag != 0) {return;}

var launch_inclination = getprop("/fdm/jsbsim/systems/ap/launch/inclination-target");

ascent_abort_boundary.two_engines_tal = SpaceShuttle.clamp(-34.63 * launch_inclination + 7587.01, 5000, 7000);
ascent_abort_boundary.rtls = -8.66 * launch_inclination + 8646.75;
ascent_abort_boundary.ato = 90.9 * launch_inclination + 6109.1;
ascent_abort_boundary.two_engines_ptm = 138.5 * launch_inclination + 7751.9;
ascent_abort_boundary.hdot_2eo = SpaceShuttle.clamp(22.51 * launch_inclination + 688.4, 1000, 2000);
ascent_abort_boundary.single_ops3 = 8.65 * launch_inclination + 11853.2;
ascent_abort_boundary.single_engine_tal = 8.65 * launch_inclination + 13853.2;
ascent_abort_boundary.single_engine_ptm = 47.6 * launch_inclination + 15142.9;
ascent_abort_boundary.no_engines_tal = -4.3 * launch_inclination + 18623.4;
ascent_abort_boundary.no_engines_aoa = 8.7 * launch_inclination + 24653.2;

print("two engines tal is : ", ascent_abort_boundary.two_engines_tal);
print("rtls is : ", ascent_abort_boundary.rtls);
print("ato is : ", ascent_abort_boundary.ato);
print("two engines ptm is : ", ascent_abort_boundary.two_engines_ptm);
print("hdot 2eo is : ", ascent_abort_boundary.hdot_2eo);
print("single engine ops3 is : ", ascent_abort_boundary.single_ops3);
print("single engine tal is : ", ascent_abort_boundary.single_engine_tal);
print("single engine ptm is : ", ascent_abort_boundary.single_engine_ptm);
print("no engines tal is : ", ascent_abort_boundary.no_engines_tal);
print("no engines aoa is : ", ascent_abort_boundary.no_engines_aoa);
...
var full_ignition = func{
    ...
    ssme_array[0].throttle_to(1.0);
    ssme_array[1].throttle_to(1.0);
    ssme_array[2].throttle_to(1.0);
...
if ((SRB_fuel < 0.05) and (SRB_message_flag == 0))
...
if ((SRB_fuel < 0.0005) and (SRB_message_flag == 1))
	{SRB_separate(); SRB_message_flag = 2;}

#No bucket if engine(s) is(are) failed
if (engop == 3)
	{
	if ((t_elapsed > (getprop("/fdm/jsbsim/systems/ap/launch/throttle-down-time-s") + 6)) and (launch_message_flag ==0) and (SRB_burn_timer >0.0))
		{
		SpaceShuttle.callout.make("Atlantis, Go at throttle down!", "real");
		launch_message_flag = 1;
		}

	if ((t_elapsed > (getprop("/fdm/jsbsim/systems/ap/launch/throttle-up-time-s") + 6)) and (launch_message_flag ==1))
		{
		SpaceShuttle.callout.make("Atlantis, Go at throttle up!", "real");
...
if (thrust_engine1 > 0.0)
	...
	if ((engine_fail_time < 0.0) and (regular_meco == 0))
		...
		setprop("/fdm/jsbsim/systems/abort/engine-fail-vi", vi);
		}
	}
...
if ((n_eng_operational == 1) and (engine2_fail_time < 0.0))
	{
...
if ((n_eng_operational == 0) and (regular_meco == 0) and (SRB_message_flag == 2))
	{
	print("Contingency abort init");
...
#Boundary messages from MCC

var vi = getprop("/fdm/jsbsim/velocities/eci-velocity-mag-fps");
var abort_mode = getprop("/fdm/jsbsim/systems/abort/abort-mode");


#3 engines or (2engines and ATO) or (2engines  and no Abort // Press to MECO case for example)
if ((engop == 3) or ((engop == 2) and (abort_mode == 3)) or ((engop == 2) and (abort_mode == 0)))
	{

	#1eo and 3eo boundaries callouts (1eo and 2eo callouts might be mixed for low inc)
	#For Low inc, PTM < SE OPS3 and High Inc PTM > SE OPS3 / hence two different flags and conditionnals
	if ((vi > ascent_abort_boundary.two_engines_tal) and (ascent_abort_boundary.abort_one_eo_boundary_flag == 0))
		{
		SpaceShuttle.callout.make("Atlantis, Two engines TAL", "real");
		ascent_abort_boundary.abort_one_eo_boundary_flag = 1;
		}

	else if ((vi > ascent_abort_boundary.rtls) and (ascent_abort_boundary.abort_one_eo_boundary_flag == 1))
		{
		SpaceShuttle.callout.make("Atlantis, Negative return", "real");
		ascent_abort_boundary.abort_one_eo_boundary_flag = 2;
		}

	else if ((vi > ascent_abort_boundary.ato) and (ascent_abort_boundary.abort_one_eo_boundary_flag == 2))
		{
		SpaceShuttle.callout.make("Atlantis, Press to A T O", "real");
		ascent_abort_boundary.abort_one_eo_boundary_flag = 3;
		}

	else if ((vi > ascent_abort_boundary.two_engines_ptm) and (ascent_abort_boundary.abort_one_eo_boundary_flag == 3))
		{
		SpaceShuttle.callout.make("Atlantis, Press to MECO", "real");
		ascent_abort_boundary.abort_one_eo_boundary_flag = 4;
		}
	else if ((vi > ascent_abort_boundary.no_engines_tal) and (ascent_abort_boundary.abort_one_eo_boundary_flag == 4))
		{
		SpaceShuttle.callout.make("Atlantis, Three engines out TAL", "real");
		ascent_abort_boundary.abort_one_eo_boundary_flag = 5;
		}
	else if ((vi > ascent_abort_boundary.no_engines_aoa) and (ascent_abort_boundary.abort_one_eo_boundary_flag == 5))
		{
		SpaceShuttle.callout.make("Atlantis, Three engines out A O A", "real");
		ascent_abort_boundary.abort_one_eo_boundary_flag = 6;
		}
	
	#2eo boundaries callouts
	if ((vi > ascent_abort_boundary.single_ops3) and (ascent_abort_boundary.abort_two_eo_boundary_flag == 0))
		{
		SpaceShuttle.callout.make("Atlantis, Single engine OPS 3 1O9", "real");
		ascent_abort_boundary.abort_two_eo_boundary_flag = 1;
		}
	else if ((vi > ascent_abort_boundary.single_engine_tal) and (ascent_abort_boundary.abort_two_eo_boundary_flag == 1))
		{
		SpaceShuttle.callout.make("Atlantis, Single engine TAL 1O4", "real");
		ascent_abort_boundary.abort_two_eo_boundary_flag = 2;
		}
	else if ((vi > ascent_abort_boundary.single_engine_ptm) and (ascent_abort_boundary.abort_two_eo_boundary_flag == 2))
		{
		SpaceShuttle.callout.make("Atlantis, Single engine press 1O4", "real");
		ascent_abort_boundary.abort_two_eo_boundary_flag = 3;
		}
	}

#2 engines TAL single engine boundaries redesignation callouts 
else if ((engop == 2) and (abort_mode == 2))
	{
	#Flag for 2EO Green/ Droop Vi boundary in contigency.nas
	ascent_abort_boundary.tal_redesignation_flag = 1;
	setprop("/fdm/jsbsim/systems/abort/boundary-redes-flag", 1);

	#Redesignation information
	if (ascent_abort_boundary.abort_two_eo_boundary_flag == 0)
		{
		SpaceShuttle.callout.make("Atlantis, Single engine boundaries redesignation, check your cue cards", "real");
		ascent_abort_boundary.abort_two_eo_boundary_flag = 1;
		}

	#2eo boundaries callouts
	else if ((vi > getprop("/fdm/jsbsim/systems/abort/boundary-one-engine-ops3-redes")) and (ascent_abort_boundary.abort_two_eo_boundary_flag == 1))
		{
		SpaceShuttle.callout.make("Atlantis, Single engine OPS 3 1O9", "real");
		ascent_abort_boundary.abort_two_eo_boundary_flag = 2;
		}
	else if ((vi > getprop("/fdm/jsbsim/systems/abort/boundary-one-engine-tal-redes")) and (ascent_abort_boundary.abort_two_eo_boundary_flag == 2))
		{
		SpaceShuttle.callout.make("Atlantis, Single engine TAL 1O4", "real");
		ascent_abort_boundary.abort_two_eo_boundary_flag = 3;
		}
	else if ((vi > ascent_abort_boundary.no_engines_tal) and (ascent_abort_boundary.abort_two_eo_boundary_flag == 3))
		{
		SpaceShuttle.callout.make("Atlantis, Three engines out TAL", "real");
		ascent_abort_boundary.abort_two_eo_boundary_flag = 4;
		}
	}
...
if ((thrust1 > 350000.0) and (thrust2 > 350000.0) and (thrust3 > 350000.0) and (hyd_pressurized == 3)) # we're go
	{
	...
	ssme_array[0].throttle_to(0.0);
	ssme_array[1].throttle_to(0.0);
	ssme_array[2].throttle_to(0.0);
...
SpaceShuttle.callout.make("Atlantis, Go at SRB SEP", "help");
...
if (getprop("/fdm/jsbsim/systems/dps/ops") == 1)
	...
	SpaceShuttle.callout.make("Atlantis, Prepare for MECO!", "help");
	}
...
SpaceShuttle.callout.make("Atlantis, SRB separation!", "info");
...
if ((n_eng_operational == 1) or (arm_serc == 1))
	{
	setprop("/fdm/jsbsim/systems/fcs/control-mode",13);
	setprop("/controls/shuttle/control-system-string", "SERC");
...
if ((math.abs(pitch_rate) > 0.013089) or (math.abs(roll_rate) > 0.02181 ) or (math.abs(yaw_rate) > 0.013089)) 
	...
	SpaceShuttle.create_fault_message("    ET SEP    "," INH", "1234 ", 2);	#Both PASS and BFS for Inh
	return;
	}
...
var update_rms_view = func {
    
    var state = getprop("/fdm/jsbsim/systems/rms/rms-powered");
...
#	print ("camera att= ",cam_eff_pitch," ",cam_eff_yaw," ",cam_eff_roll," ");
#	print ("camera pos= ",cam_eff_x," ",cam_eff_y," ",cam_eff_z," ");
...
if (alt < 400000.0)
	...
	#MECO below 400 kfeet with new ascent guidance (July 2020) // Entry interface help message for OPS 3 only to avoid message and deorbit loop at MECO

	if ((periapsis < 100.0) and (ops !=1))
		{
...
if ((current_mode == 0) or (current_mode == 10) or (current_mode == 13))
	{
...
#In case of Bounce Entry into atmosphere ( Low energy case// full L/D // no bank// phugoid), it allowed to have RCS roll control again at low q bar
else if ((qbar < 10.0) and (deorbit_stage_flag == 1)) 
	{
	setprop("/fdm/jsbsim/systems/fcs/rcs-roll-mode", 1);
	deorbit_stage_flag = 0;
	}


#Q bar 20 for elevons and Body flaps into pitch channel 
...
#Mach 4 for Rudder active for control and trim
...
#Improved TAEM transition logic (OPS 3 vs OPS 6) taking account minor differences between GRTLS TAEM vs OPS 3 TAEM logic

if (ops == 6) #OPS 6 GRTLS and TAEM
	{
	
	#Update entry guidance loop for distance to runway in mm 602 and 603
	if (guidance_mode > 0) {SpaceShuttle.update_entry_guidance();}
		
	#Altitude and speed trigger for TAEM (OPS6 or Contigency) // Transition at mach 3.2 in GRTLS or 100 Nm  to avoid high energy situations
	#Deorbit flag more than 1 (yaw rcs not always deactivated for early RTLS transition // done in glideloop later on for OPS 6)

	if (((altitude < 100000.0) or (mach < 3.2) or (d_remain < 100)) and (deorbit_stage_flag > 1) and (Nz_hold == 0) and (alpha_recovery == 0))
		{
...
else if (ops == 3)  #OPS 3 TAEM
	{
	# switch to TAEM guidance 60 miles/ mach 2.5/ 85kfeet from site through wp1 if we're under entry guidance
	# we never do this for a contingency or RTLS abort (OPS6)
	...
	if (guidance_mode > 0)
		...
		if (((d_remain < 60.0) or (mach < 2.5)) and (deorbit_stage_flag == 3))
			...
			#setprop("/sim/messages/copilot", "TAEM interface reached.");
			SpaceShuttle.callout.make("TAEM interface reached.", "info");
			SpaceShuttle.callout.make("Check NH3 boiler operation!", "help");
			setprop("/controls/shuttle/hud-mode",3);
...
#Yaw rcs deactivation conditionnal for OPS 6 with earlier TAEM handover above mach 3.5
if ((getprop("/fdm/jsbsim/velocities/mach") < 3.5) and (deorbit_stage_flag == 2))
	{
	setprop("/fdm/jsbsim/systems/fcs/rcs-yaw-mode", 0);
	setprop("/fdm/jsbsim/systems/fcs/no-y-jet", 0);
...
#print (getprop("/position/latitude-deg"), " ", getprop("/position/longitude-deg"), " ", getprop("/position/altitude-ft"), " ", getprop("/fdm/jsbsim/velocities/vtrue-kts"));
...
var shuttle_main_engine = {
	...
	new: func (index) {
	 	var e = { parents: [shuttle_main_engine] };
	
		e.index = index;
		return e;
		},

	throttle_to: func (value) {
		
		if ((me.command_path == 1) and (me.lockup_state == 1))
			{
			me.throttle_setting = value;
			#setprop("/controls/engines/engine["~me.index~"]/throttle", value);
			setprop("/controls/engines/engine["~me.index~"]/ap-throttle-cmd", value);
			}
	...
	ignite: func {
		...
		if (me.command_path == 1)
			{
			setprop("/fdm/jsbsim/systems/mps/engine["~me.index~"]/run-cmd", 1);
			me.run_cmd = 1;
			}
	},
	...
	cutoff: func {
		...
		if (me.command_path == 1)
			{
			setprop("/fdm/jsbsim/systems/mps/engine["~me.index~"]/run-cmd", 0);
			me.run_cmd = 0;
			}
	},
...
var ssme_left = shuttle_main_engine.new(0);
var ssme_right = shuttle_main_engine.new(1);
var ssme_center = shuttle_main_engine.new(2);

var ssme_array = [ssme_left, ssme_right, ssme_center];
...
#Sb state 0.05 step to match better new SB Cl and Cd tables in shuttle.xml 

var increase_speedbrake = func {

var speedbrake_state = getprop("/fdm/jsbsim/systems/fcs/speedbrake-cmd-norm");

speedbrake_state = speedbrake_state+ 0.05;
...
# More sensitivity ( every 2.8 ° for better Cm management during early entry ---> far less fuel wasted to hold AOA until Qbar of 20)

if (bodyflap_state == -0.192) {bodyflap_state = -0.165;}
#else if (bodyflap_state == -0.265) {bodyflap_state = -0.215;}
#else if (bodyflap_state == -0.215) {bodyflap_state = -0.165;}
else if (bodyflap_state == -0.165) {bodyflap_state = -0.115;}
else if (bodyflap_state == -0.115) {bodyflap_state = -0.065;}
else if (bodyflap_state == -0.065) {bodyflap_state = -0.025;}
else if (bodyflap_state == -0.025) {bodyflap_state = 0.0;}
else if (bodyflap_state == 0.0) {bodyflap_state = 0.025;}
else if (bodyflap_state == 0.025) {bodyflap_state = 0.065;}
else if (bodyflap_state == 0.065) {bodyflap_state = 0.115;}
else if (bodyflap_state == 0.115) {bodyflap_state = 0.165;}
else if (bodyflap_state == 0.165) {bodyflap_state = 0.215;}
else if (bodyflap_state == 0.215) {bodyflap_state = 0.265;}
else if (bodyflap_state == 0.265) {bodyflap_state = 0.315;}
else if (bodyflap_state == 0.315) {bodyflap_state = 0.365;}
else if (bodyflap_state == 0.365) {bodyflap_state = 0.384;}
...
if (bodyflap_state == 0.384) {bodyflap_state = 0.365;}
else if (bodyflap_state == 0.365) {bodyflap_state = 0.315;}
else if (bodyflap_state == 0.315) {bodyflap_state = 0.265;}
else if (bodyflap_state == 0.265) {bodyflap_state = 0.215;}
else if (bodyflap_state == 0.215) {bodyflap_state = 0.165;}
else if (bodyflap_state == 0.165) {bodyflap_state = 0.115;}
else if (bodyflap_state == 0.115) {bodyflap_state = 0.065;}
...
else if (bodyflap_state == -0.165) {bodyflap_state = -0.192;}
#else if (bodyflap_state == -0.215) {bodyflap_state = -0.265;}
#else if (bodyflap_state == -0.265) {bodyflap_state = -0.315;}
...
#Not used
var bodyflap_trail = func {

var bodyflap_state = getprop("/controls/shuttle/bodyflap-pos-rad");

if (bodyflap_state != 0.0) {bodyflap_state = 0.0;}
...
if (n == 0)
	{
	if ((SpaceShuttle.failure_cmd.ssme_pb1a + SpaceShuttle.failure_cmd.ssme_pb1b) == 1)
		{
		flag = 0;
		if (cws_msg_hash.me_shdn_sw_l == 0)
			{
			create_fault_message("    ME SHDN SW","   L","1234 ", 2, "PASS"); #PASS only	
			cws_msg_hash.me_shdn_sw_l = 1;
			}
		}
	else if ((SpaceShuttle.failure_cmd.ssme_pb1a + SpaceShuttle.failure_cmd.ssme_pb1b) == 0)
		{
		flag = 0;
		}
	}
	...
	{
	if ((SpaceShuttle.failure_cmd.ssme_pb2a + SpaceShuttle.failure_cmd.ssme_pb2b) == 1)
		{
		flag = 0;
		if (cws_msg_hash.me_shdn_sw_r == 0)
			{
			create_fault_message("    ME SHDN SW","   R","1234 ", 2, "PASS");	
			cws_msg_hash.me_shdn_sw_r = 1;
			}
		}
	else if ((SpaceShuttle.failure_cmd.ssme_pb2a + SpaceShuttle.failure_cmd.ssme_pb2b) == 0)
		{
		flag = 0;
		}
	}
	...
	{
	if ((SpaceShuttle.failure_cmd.ssme_pb3a + SpaceShuttle.failure_cmd.ssme_pb3b) == 1)
		{
		flag = 0;
		if (cws_msg_hash.me_shdn_sw_c == 0)
			{
			create_fault_message("    ME SHDN SW","   C","1234 ", 2, "PASS");
			cws_msg_hash.me_shdn_sw_c = 1;
			}
		}
	else if ((SpaceShuttle.failure_cmd.ssme_pb3a + SpaceShuttle.failure_cmd.ssme_pb3b) == 0)
		{
		flag = 0;
		}
	}
...
var ssme_lockup = func (n) {
    ...
    if (n==0) {number = 2; SpaceShuttle.failure_cmd.ssme1 = 0; ssme_array[0].lockup_state = 0;}
    if (n==1) {number = 3; SpaceShuttle.failure_cmd.ssme2 = 0; ssme_array[1].lockup_state = 0;}
    if (n==2) {number = 1; SpaceShuttle.failure_cmd.ssme3 = 0; ssme_array[2].lockup_state = 0;}
...
var deploy_chute = func {
	...
	#If SSME nozzle are not correctly stowed ( -0.45 rad), drag chute can't be deployed
	if ((getprop("/fdm/jsbsim/propulsion/engine[0]/pitch-angle-rad-cmd") == 0.0) or (getprop("/fdm/jsbsim/propulsion/engine[1]/pitch-angle-rad-cmd") == 0.0) or (getprop("/fdm/jsbsim/propulsion/engine[2]/pitch-angle-rad-cmd") == 0.0)) 
		{
		SpaceShuttle.callout.make("Chute can only be deployed if SSME nozzles are stowed correctly!", "help");
		return;
		}
...
if (SpaceShuttle.dps_simulation_detail_level == 1)
...
else	# force PASS into BFS MM,  mode all GPCs from standby back to run
	{
	
	# mode the non-BFS GPCs into software-controlled run

	var mm_bfs = getprop("/fdm/jsbsim/systems/dps/major-mode-bfs");
	setprop("/fdm/jsbsim/systems/dps/major-mode", mm_bfs);
	...
	if ((mm_bfs == 104) or (mm_bfs == 301))
		{
		ops_transition_auto("p_dps_mnvr");
		}		
...
#MPS TVC open for SSME repositionning in scenarios post OPS3
setprop("/fdm/jsbsim/systems/apu/apu[0]/tvc-isolation-valve-status", 1);
setprop("/fdm/jsbsim/systems/apu/apu[1]/tvc-isolation-valve-status", 1);
setprop("/fdm/jsbsim/systems/apu/apu[2]/tvc-isolation-valve-status", 1);
...
# initialize pressure high enough to suppress CWS failure message

setprop("/fdm/jsbsim/systems/apu/apu/circ-pressure-factor", 0.95);
setprop("/fdm/jsbsim/systems/apu/apu[1]/circ-pressure-factor", 0.95);
setprop("/fdm/jsbsim/systems/apu/apu[2]/circ-pressure-factor", 0.95);
...
#For TAEM and AL scenarios free flight without deorbit_loop runned, as no deorbit loop (RCS jets need to be turned off) 
var rcs_aerodap_off = func {
setprop("/fdm/jsbsim/systems/fcs/rcs-roll-mode", 0);
setprop("/fdm/jsbsim/systems/fcs/rcs-pitch-mode", 0);
setprop("/fdm/jsbsim/systems/fcs/rcs-yaw-mode", 0);
...
if (stage == 0)
	...
	# body flap talkback indicates MPS dump status 
	setprop("/controls/shuttle/bf-mps-override", 1);
	setprop("/controls/shuttle/bf-mps-indicator", 1);
	
	settimer( func SpaceShuttle.light_manager.set_theme("PAD"), 5.0);

	# show MET instead of GMT for liftoff
...
if (stage == 2) 
	...
	hud_covers_off();
	hydraulics_on(); #Correct SSME position 
	area_nav_on(); #Tacan, MLS ,RA on
...
if (stage == 3) 
	...
	hud_covers_off();
	hydraulics_on(); #Correct SSME position 
	rcs_aerodap_off(); #Inhibit RCS jets if no guidance loaded (free flight)
	
	
	#NH3 Boiler to avoid Freon Alarm
	setprop("/fdm/jsbsim/systems/atcs/nh3-controller-A-switch", -1);
...
if (stage == 4) 
	...
	#11Nm // 17kft QFE for a 19° path
	setprop("/position/altitude-ft", 13000 + place_alt_correct);
...
if (stage == 6) 
	...
	setprop("/controls/shuttle/bf-mps-override", 1);
	setprop("/controls/shuttle/bf-mps-indicator", 0);
...
#listener for the SRMS view. Let's start it only when the SRMS is powered
setlistener("/fdm/jsbsim/systems/rms/rms-powered", func {update_rms_view();},0,0);
...
if (getprop("/sim/presets/stage") == 2) # we start with entry
	...
	setprop("/consumables/fuel/tank[4]/level-lbs",1554.0);
	setprop("/consumables/fuel/tank[5]/level-lbs",943.0);
	setprop("/consumables/fuel/tank[6]/level-lbs",1554.0);
	setprop("/consumables/fuel/tank[7]/level-lbs",943.0);
...
if (getprop("/sim/presets/stage") == 7) # Grenadier: runway, gear down, no stack, cold engines
	...
	print("Plan A high-AR FDM: Sw≈9688 ft2, b≈197 ft (60 m), empty≈378534 lb (~172 t dry)");
	}
