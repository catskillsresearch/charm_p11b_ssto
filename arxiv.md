...
**A stylized listen.** [`charm_ssto_ascent_soundscape.wav`](research/figures/audio/charm_ssto_ascent_soundscape.wav) (repo, \(\sim 52\,\mathrm{s}\) stereo, generated by [`emit_audio_signature.py`](research/figures/cad/emit_audio_signature.py)) is a **time-compressed sound-design pass**, not a validated psychoacoustic or CFD acoustic simulation: real stage durations are \(t_1\approx 133\,\mathrm{s}\), \(t_2\approx 29\,\mathrm{min}\), \(t_3\approx 4.3\,\mathrm{h}\) (§10.5/§10.6), obviously not literal audio length. What *is* grounded in the numbers above: the two "roars" really are mixed at the \(\dot m\,v_{\mathrm{jet}}^{\,n}\) ratio from the table (dynamic range compressed \(2\times\) for audibility, not hidden), and a stylized N-wave "crack" transient sits exactly at the timeline fraction corresponding to the \(M=1\) crossing computed above. Frequencies and timbre are illustrative — no fan blade count, duct geometry, or Strouhal-scaled spectrum exists for this airframe to compute a real one — but each stage is still built from a distinct, motivated mechanism rather than one flat noise wash: Stage 1 spools up from standstill and builds with ground-roll speed toward \(V_{\mathrm{lof}}\) (§10.3), with a faint high-frequency inverter/PWM whine (visible as the wavering band near \(7\,\mathrm{kHz}\) in the figure below) marking it as *electric*, not turbine; Stage 2 adds a sizzling plasma-discharge crackle and a slow turbulent "surge" on top of the broadband roar, since a real microwave/arc air-plasma jet buzzes and spits rather than just hissing like a clean thermal jet. The final segment renders the Stage-3 vacuum silence from the cabin's perspective: no jet roar reaches the hull once there is no air outside it to carry one — but that does **not** mean nothing is audible. It is deliberately a **"bump and grind,"** not a hum alone: magnet/pump tonal whine, a low broadband bearing-friction "grind" texture, and a periodic percussive cold-head knock (synthesized as a short decaying broadband impulse at the cryocooler's own cycle rate, not a smooth sine swell — a real Gifford-McMahon cold head's displacer stroke is a knock, per §9.5/§9.6), all mixed well below the atmospheric-flight levels above but with real presence, not near-silence.
...
### 10.9 Orbital ascent profile

To make the three-stage flight envelope of §10.3–10.6 concrete end to end, Fig.~\ref{fig:orbital-ascent-profile} traces the reference mission from a specific, real municipal field — **Statesville Regional Airport (KSVH)**, Iredell County, NC (formerly *Statesville Municipal Airport*) — up to ISS-altitude LEO. This is a **schematic**, not a navigation product: the spiral panel's radial axis exaggerates altitude relative to Earth's radius for legibility (true LEO-altitude/\(R_\oplus\approx 0.06\), an invisible sliver at true scale), and Stage 1/Stage 3 altitude-vs-time *pacing* is a smooth interpolation between grounded endpoints rather than a separately time-integrated trajectory (only Stage 2 has a real \(h(t)\), reusing the exact constant-\(Q\) quadrature from `integrate_stage2_climb` in §10.4). What *is* physically real: the three stage durations (\(t_1\approx 133\,\mathrm{s}\), \(t_2\approx 28.7\,\mathrm{min}\), \(t_3\approx 4.33\,\mathrm{h}\)) are the same numbers from §10.5/§10.6, Stage 1 ends exactly at \(h(v_1)\) on the same constant-\(Q\) schedule Stage 2 continues (not a new assumption), and the spiral's rotation rate at each instant is the true local circular-orbit rate \(2\pi/T(h)\) from the vis-viva relation — so the \(\approx 3.3\) visible loops during Stage 3 are a genuine consequence of a \(\sim 4.3\,\mathrm{h}\) burn against an \(\sim 88\)–\(93\,\mathrm{min}\) LEO period in this altitude band, not a decorative flourish. Note KSVH's real runway (\(2{,}135\,\mathrm{m}\)) is shorter than the \(3{,}500\,\mathrm{m}\) municipal-runway closure assumed for the reference GLOW elsewhere in this paper (§10.3, G6) — it is used here as a concrete, real-world stand-in for "a municipal airport," not a claim that this exact field supports the takeoff roll as sized.

![Schematic ascent profile from Statesville Regional Airport (KSVH), NC to ISS-altitude LEO (\(\approx 400\,\mathrm{km}\), \(51.6^\circ\) [15]): spiral view (left, altitude exaggerated, colored by stage) and altitude vs. mission elapsed time on a log time axis (right, true relative stage durations). Generated by `emit_orbital_ascent_diagram.py`.](research/figures/orbital_ascent_profile.png)
