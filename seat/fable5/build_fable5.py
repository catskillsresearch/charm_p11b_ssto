...
``../spacex_crew_dragon_seat.png`` — fable5 variant.
...
pivot collar, and the footrest hinges from the pan clevises.

Key difference from the composer_25 variant: the footrest is ONE contiguous
molded carbon piece — a flat picture-frame slab with rounded corners and two
integrated support arms, boolean-unioned into a single watertight mesh — not
a loop of tube stock with separate rod arms. Headrest wing cushions are
included and visible in the renders.
...
Hierarchy::
    ...
    │   ├── Shell_*, Cushion_* (incl. headrest wing cushions), Trim, Arm_Pads
    │   └── Footrest_Assembly
    │       ├── Clevis_*, Hinge_Pin_*
    │       └── Footrest_Molded   (single contiguous frame + arms mesh)

Run::

    /snap/bin/blender -b -P seat/fable5/build_fable5.py

Outputs ``seat/fable5/fable5.blend`` and verification renders.
"""
...
BLEND_OUT = SEAT_DIR / "fable5.blend"
PNG_OUT = SEAT_DIR / "fable5_render.png"
PNG_OUT_SIDE = SEAT_DIR / "fable5_render_side.png"
PNG_OUT_MOUNT = SEAT_DIR / "fable5_render_mount.png"
...
def mesh_island_count(ob: bpy.types.Object) -> int:
    """Count disconnected vertex islands in a mesh (1 == contiguous)."""
    bm = bmesh.new()
...
def build_footrest(parent: bpy.types.Object, c_foot, mats) -> list[bpy.types.Object]:
    """Single contiguous molded carbon footrest hinged at FOOTREST_ATTACH_*.

    The frame is a flat picture-frame slab (rounded-rect outer profile with a
    rounded-rect cutout) and the two support arms are flattened blades that
    are boolean-unioned into the frame, yielding ONE watertight mesh — a
    molded piece rather than tube stock and rods.
    """
    objs = []
    m_c, m_m = mats["carbon"], mats["metal_dark"]

    frame_center = Vector((0.0, 0.82, -0.15))
    tilt = math.radians(52.0)
    outer_w, outer_h = 0.50, 0.64
    band = 0.062       # width of the flat frame face
    thick = 0.024      # molded slab thickness
    corner_r = 0.085

    def frame_pt(xl: float, yl: float) -> Vector:
        """Frame-local (x, y) in the slab plane → world point."""
        return frame_center + Vector((xl, yl * math.cos(tilt), yl * math.sin(tilt)))

    def rounded_rect(hw: float, hh: float, cr: float, n_corner: int = 10):
        pts = []
        corners = [
            (hw - cr, hh - cr, 0.0),
        ...
        for cx, cy, a0 in corners:
            for i in range(n_corner):
                ...
                pts.append(Vector((cx + cr * math.cos(a), cy + cr * math.sin(a), 0.0)))
        return pts

    outer = rounded_rect(outer_w / 2, outer_h / 2, corner_r)
    inner = rounded_rect(outer_w / 2 - band, outer_h / 2 - band, max(corner_r - band * 0.7, 0.022))
    ...
    bm.normal_update()
    mesh = bpy.data.meshes.new("Footrest_Molded")
    bm.to_mesh(mesh)
    bm.free()
    frame = bpy.data.objects.new("Footrest_Molded", mesh)
    c_foot.objects.link(frame)
    assign(frame, m_c)
    frame.location = frame_center
    frame.rotation_euler = (tilt, 0.0, 0.0)
    add_solidify(frame, thick, offset=0.0)
    add_bevel(frame, width=0.006, segments=3)

    # Integrated support arms: flattened blades from the pan clevises into the
    # frame band, overshooting both ends so the boolean union is watertight.
    arm_tmp = []
    for side, sign, attach in (("L", 1.0, FOOTREST_ATTACH_L), ("R", -1.0, FOOTREST_ATTACH_R)):
        clevis = make_box(f"Footrest_Clevis_{side}", (0.040, 0.046, 0.022), attach, m_c, c_foot,
                          rotation=(math.radians(8), 0, 0), parent=parent)
        objs.append(clevis)
        pin = make_cylinder(f"Footrest_Pin_{side}", 0.009, 0.056, attach, m_m, c_foot, axis="X", parent=parent, verts=20)
        objs.append(pin)

        p0 = Vector((sign * 0.170, 0.375, -0.006))
        p1 = frame_pt(sign * 0.150, -0.255)  # near-band slab mid-plane
        direction = (p1 - p0).normalized()
        # Extend into the clevis; stop at the slab mid-plane so the joint is
        # buried inside the molding instead of poking out the back face.
        start = p0 - direction * 0.05
        delta = p1 - start
        length = delta.length
        mid = start + delta * 0.5
        bpy.ops.mesh.primitive_cube_add(size=1.0, location=mid)
        arm = bpy.context.active_object
        arm.name = f"Footrest_ArmBlade_{side}"
        arm.scale = (0.055, length, 0.022)
        bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)
        arm.rotation_euler = delta.normalized().to_track_quat("Y", "Z").to_euler()
        assign(arm, m_c)
        link_only(arm, c_foot)
        add_bevel(arm, width=0.004, segments=2)
        arm_tmp.append(arm)

    for arm in arm_tmp:
        mod = frame.modifiers.new("Union", "BOOLEAN")
        mod.operation = "UNION"
        mod.object = arm
    ...
    shade_auto_smooth(frame, 32)
    parent_keep(frame, parent)
    objs.append(frame)

    islands = mesh_island_count(frame)
    print(f"[fable5] Footrest_Molded islands: {islands} "
          f"({'contiguous' if islands == 1 else 'WARNING: not contiguous!'})")
    return objs
